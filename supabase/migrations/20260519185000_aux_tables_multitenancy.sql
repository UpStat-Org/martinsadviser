...
DROP POLICY IF EXISTS "users delete own comments in org" ON public.comments;
...
CREATE INDEX IF NOT EXISTS idx_saved_filters_org_id ON public.saved_filters(org_id);
...
DROP POLICY IF EXISTS "users read own filters in org" ON public.saved_filters;
CREATE POLICY "users read own filters in org"
  ON public.saved_filters FOR SELECT TO authenticated
  USING (auth.uid() = user_id AND public.is_org_member(org_id));

DROP POLICY IF EXISTS "users insert own filters in org" ON public.saved_filters;
...
DROP POLICY IF EXISTS "users update own filters in org" ON public.saved_filters;
CREATE POLICY "users update own filters in org"
  ON public.saved_filters FOR UPDATE TO authenticated
  USING (auth.uid() = user_id AND public.is_org_member(org_id))
  WITH CHECK (auth.uid() = user_id AND public.is_org_member(org_id));

DROP POLICY IF EXISTS "users delete own filters in org" ON public.saved_filters;
...
CREATE INDEX IF NOT EXISTS idx_message_templates_org_id ON public.message_templates(org_id);
...
DROP POLICY IF EXISTS "users read own templates in org" ON public.message_templates;
CREATE POLICY "users read own templates in org"
  ON public.message_templates FOR SELECT TO authenticated
  USING (auth.uid() = user_id AND public.is_org_member(org_id));

DROP POLICY IF EXISTS "users create own templates in org" ON public.message_templates;
CREATE POLICY "users create own templates in org"
  ON public.message_templates FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id AND public.is_org_member(org_id));

DROP POLICY IF EXISTS "users update own templates in org" ON public.message_templates;
CREATE POLICY "users update own templates in org"
  ON public.message_templates FOR UPDATE TO authenticated
  USING (auth.uid() = user_id AND public.is_org_member(org_id))
  WITH CHECK (auth.uid() = user_id AND public.is_org_member(org_id));

DROP POLICY IF EXISTS "users delete own templates in org" ON public.message_templates;
...
DROP POLICY IF EXISTS "users read own messages in org" ON public.scheduled_messages;
CREATE POLICY "users read own messages in org"
  ON public.scheduled_messages FOR SELECT TO authenticated
  USING (auth.uid() = user_id AND public.is_org_member(org_id));

DROP POLICY IF EXISTS "users create own messages in org" ON public.scheduled_messages;
CREATE POLICY "users create own messages in org"
  ON public.scheduled_messages FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id AND public.is_org_member(org_id));

DROP POLICY IF EXISTS "users update own messages in org" ON public.scheduled_messages;
CREATE POLICY "users update own messages in org"
  ON public.scheduled_messages FOR UPDATE TO authenticated
  USING (auth.uid() = user_id AND public.is_org_member(org_id))
  WITH CHECK (auth.uid() = user_id AND public.is_org_member(org_id));

DROP POLICY IF EXISTS "users delete own messages in org" ON public.scheduled_messages;
...
CREATE INDEX IF NOT EXISTS idx_client_internal_notes_org_id ON public.client_internal_notes(org_id);
...
DROP POLICY IF EXISTS "org members read internal notes" ON public.client_internal_notes;
CREATE POLICY "org members read internal notes"
  ON public.client_internal_notes FOR SELECT TO authenticated
  USING (public.is_org_member(org_id));

DROP POLICY IF EXISTS "org members insert internal notes" ON public.client_internal_notes;
...
DROP POLICY IF EXISTS "org members update internal notes" ON public.client_internal_notes;
CREATE POLICY "org members update internal notes"
  ON public.client_internal_notes FOR UPDATE TO authenticated
  USING (public.is_org_member(org_id))
  WITH CHECK (public.is_org_member(org_id));

DROP POLICY IF EXISTS "org members delete internal notes" ON public.client_internal_notes;
