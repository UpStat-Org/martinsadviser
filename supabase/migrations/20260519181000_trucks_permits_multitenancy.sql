...
CREATE INDEX IF NOT EXISTS idx_trucks_org_id ON public.trucks(org_id);
...
DROP POLICY IF EXISTS "org members create trucks" ON public.trucks;
...
DROP POLICY IF EXISTS "org members update trucks" ON public.trucks;
CREATE POLICY "org members update trucks"
  ON public.trucks FOR UPDATE TO authenticated
  USING (public.is_org_member(org_id))
  WITH CHECK (public.is_org_member(org_id));

DROP POLICY IF EXISTS "org members delete trucks" ON public.trucks;
...
CREATE INDEX IF NOT EXISTS idx_permits_org_id ON public.permits(org_id);
...
DROP POLICY IF EXISTS "org members read permits" ON public.permits;
CREATE POLICY "org members read permits"
  ON public.permits FOR SELECT TO authenticated
  USING (public.is_org_member(org_id));

DROP POLICY IF EXISTS "org members create permits" ON public.permits;
...
DROP POLICY IF EXISTS "org members update permits" ON public.permits;
CREATE POLICY "org members update permits"
  ON public.permits FOR UPDATE TO authenticated
  USING (public.is_org_member(org_id))
  WITH CHECK (public.is_org_member(org_id));

DROP POLICY IF EXISTS "org members delete permits" ON public.permits;
...
CREATE INDEX IF NOT EXISTS idx_permit_history_org_id ON public.permit_history(org_id);
...
DROP POLICY IF EXISTS "org members read permit history" ON public.permit_history;
CREATE POLICY "org members read permit history"
  ON public.permit_history FOR SELECT TO authenticated
  USING (public.is_org_member(org_id));

DROP POLICY IF EXISTS "org members insert permit history" ON public.permit_history;
...
CREATE INDEX IF NOT EXISTS idx_permit_documents_org_id ON public.permit_documents(org_id);
